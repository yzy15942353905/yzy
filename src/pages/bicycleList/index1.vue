...
export default {
  ...
  methods: {
    ...
    //商品属性（排他）
    selectSpuSaleAttr(obj) {
      this.$store.dispatch("changeAttrSale", obj);
    },

    //加入购物车
    // async addShopCart() {
    //   let result = this.$store.dispatch("addToShopCart", {
    ...
    //     element.spuSaleAttrValueList.forEach((ele) => {
    //       if (ele.isChecked == 1)
    //         attrList.push([element.saleAttrName, ele.saleAttrValueName]);
    //     });
    //   });
    ...
    addShopCart: _.debounce(function () {
      let result = this.$store.dispatch("addToShopCart", {
        skuId: this.$route.params.skuId,
        num: this.num,
      });
